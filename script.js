...
// Inisialisasi banner pertama
if (banners.length > 0) {
    banners.forEach(banner => banner.style.left = '-100%');
    banners[currentBanner].classList.add('active');
    setInterval(showNextBanner, 2000); // Ganti gambar setiap 2 detik
}
...
function generateReceipt(data) {
    try {
        ...
        doc.save(`Biodata ${data.name}.pdf`); // Nama file diperbaiki
    } catch (error) {
...
function generateBoardingPass(name, airline, seat, bookingCode) {
    try {
        ...
        doc.save(`boarding_pass_${name}.pdf`); // Nama file diperbaiki
    } catch (error) {
